...
In this case, we can replace $Z(\alpha_t)$ in the identity above by $\alpha^t$,
at least for $t<T$. 
...
both increasing the cost of a single iteration, and increasing the total number
...
above, but through more involved steps, which involves the CDF of random
